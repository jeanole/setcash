...
function renderFilteredBills() {
    ...
    <tr data-id="${bill.id}" class="clickable-row cursor-pointer${bill.status === "draft" ? " bill-draft-row" : ""}" onclick="openBillDetail(${bill.id})">
      ...
      <td class="px-3 py-2.5 flex items-center gap-1.5 flex-wrap">${renderOcrBadge(bill)}${projectOcrEnabled && bill.images && bill.images.length && bill.ocrStatus !== "pending" ? `<button class="text-xs px-2 py-1 bg-amber-50 text-amber-700 rounded-md hover:bg-amber-100 transition-colors border border-amber-200 cursor-pointer whitespace-nowrap" onclick="event.stopPropagation(); triggerBillAnalysisFromList(${bill.id})">${bill.ocrStatus === "done" ? "Re-analyse" : "Analyse"}</button>` : ""}<button class="text-xs px-2.5 py-1 bg-indigo-50 text-indigo-600 rounded-md hover:bg-indigo-100 transition-colors border-none cursor-pointer" onclick="event.stopPropagation(); openBillDetail(${bill.id})">View</button></td>
