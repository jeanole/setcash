...
export default function BillImageUpload({
  ...
  // Stable object URLs for selected file previews — revoked on change
  ...
  const validateFiles = (newFiles: FileList | null): File[] => {
    ...
    const totalCount =
      ...
      validFiles.length +
      processedFilesRef.current.length;
    if (totalCount > maxFiles) {
      newErrors.push(`Maximum ${maxFiles} images allowed per bill`);
      const allowedNew = Math.max(
        0,
        maxFiles - selectedFiles.length - existingImages.length - processedFilesRef.current.length
      );
  ...
  const processThroughCropModal = (newFiles: File[]) => {
  ...
  const processNextFile = () => {
    ...
    if (nextIndex >= filesToProcess.length) {
      ...
      if (processed.length > 0) {
        onSelectedFilesChange([...selectedFiles, ...processed]);
      }
    } else {
    ...
    <div className="space-y-4">
      ...
      <div
        ...
        className={cn(
          ...
          isDragging
            ? 'border-[var(--vb-accent)] bg-indigo-50'
      ...
      >
        ...
        <svg
          className={cn(
            ...
            isDragging ? 'text-[var(--vb-accent)]' : 'text-slate-300'
          )}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
