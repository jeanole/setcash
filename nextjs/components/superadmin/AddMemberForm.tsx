      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[var(--vb-accent)] focus:border-[var(--vb-accent)] text-sm"
            disabled={isLoading}
          />
          ...
          <div className="flex flex-wrap gap-2">
            {(['user', 'admin', 'owner'] as RoleType[]).map((r) => (
              <button
                ...
                type="button"
                onClick={() => setRole(r)}
                className={`transition-all ${role === r ? 'ring-2 ring-offset-1 ring-[var(--vb-accent)]' : ''}`}
              >
                <RoleBadge role={r} />
              </button>
            ))}
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[var(--vb-accent)] focus:border-[var(--vb-accent)] text-sm bg-white"
            disabled={isLoading}
          >
        ...
        <div className="flex items-center justify-end gap-2 pt-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-[var(--vb-accent)] text-white font-medium rounded-lg transition-colors hover:bg-[var(--vb-accent-hover)] disabled:opacity-50 flex items-center gap-2 text-sm"
          >
            {isLoading ? (
              <>
