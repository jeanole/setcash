          ...
          <ol className="space-y-2 list-none pl-0">
            {currentGuideSteps.map((item, index) => (
              <li key={index} className="flex gap-2">
                <span className="font-bold text-zinc-800 shrink-0">{index + 1}.</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--vb-accent)] focus:ring-offset-2 ${
              ocrEnabled ? 'bg-[var(--vb-accent)]' : 'bg-slate-200'
            }`}
          >
            <span
        ...
        <div>
          ...
          <select
            ...
            disabled={!ocrEnabled}
            className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[var(--vb-accent)] focus:border-[var(--vb-accent)] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {PROVIDERS.map((p) => (
